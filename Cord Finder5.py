...
def parse_profile_text(profile_text):
    ...
    coordinates, name, power, kill_points, alliance, coalition = '', '', '', '', '', ''
    for line in lines:
        line = line.strip()
        if 'X' in line and 'Y' in line:
            ...
            power = line
        elif 'Kill Points' in line:
            kill_points = line
        elif line.startswith('Alliance'):
            alliance = line
        elif line.startswith('Coalition'):
            coalition = line
    return coordinates, name, power, kill_points, alliance, coalition

def send_to_sheet(profile_text):
    worksheet_name = "Sheet1"
    wks = sh.worksheet(worksheet_name)
    coordinates, name, power, kill_points, alliance, coalition = parse_profile_text(profile_text)
    
    wks.update_acell("A2", coordinates)
    wks.update_acell("B2", name)
    wks.update_acell("C2", power)
    wks.update_acell("D2", kill_points)
    wks.update_acell("E2", alliance)
    wks.update_acell("F2", coalition)
    
    print(f"Updated cell A2 with the value '{coordinates}'")
    print(f"Updated cell B2 with the value '{name}'")
